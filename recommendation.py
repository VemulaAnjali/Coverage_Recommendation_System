...
def recommend_agents_for_policy(customer_state, policy_type, agents_df):
    top_n = 1
    # Step 1: Strictest filter → specialization & region
    exact_match = agents_df[
        (agents_df['specialization'].str.contains(str(policy_type), case=False, na=False)) &
        (agents_df['region'].str.contains(str(customer_state), case=False, na=False))
    ].copy()
    
    # Step 2: If not enough, relax → specialization only
    if len(exact_match) < top_n:
        spec_only = agents_df[
            agents_df['specialization'].str.contains(str(policy_type), case=False, na=False)
        ].copy()
    else:
        spec_only = exact_match

    # Step 3: If still not enough → fall back to ALL agents
    if len(spec_only) < top_n:
        final_candidates = agents_df.copy()
    else:
        final_candidates = spec_only

    # Ranking score
    final_candidates['score'] = (
        final_candidates['success_rate'] * 0.6 +
        final_candidates['average_rating'] * 0.4
    )

    final_candidates = final_candidates.sort_values(by='score', ascending=False)

    return final_candidates.head(top_n)[[
        'agent_id','agent_name','region','specialization',
        'experience_years','success_rate','average_rating',
        'active_clients','languages','contact_number','email'
    ]].to_dict(orient='records')



def recommend_policies_with_agents(new_customer, top_n=2, n_clusters=4):
    ...
    for pid in top_cosine_policies:
        ...
        if agents_df is not None:
            print(new_customer.get('state','nan'))
            policy_details['recommended_agents'] = recommend_agents_for_policy(
                new_customer.get('state','nan'), pid, agents_df
            )
